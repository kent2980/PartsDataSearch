...
public class RemainingPartsMathModel {
	...
	private final Map<Integer, String> partsStreamSet;

	public RemainingPartsMathModel(LocalDateTime fromtime, Map<Integer, String> partsStreamSet){
	...
	public int getRemainingPartsNumber() {
		//パーツリストを取得
		Map<Integer, String> partsMap = partsStreamSet;
		List<String> l2 = partsMap.entrySet().stream()
											 .map(m -> m.getValue())
											 .distinct()
	...
	public Object[][] getRemainingPartsList(){
		...
		//◆ パーツリストを取得
		Map<Integer, String> partsMap = partsStreamSet;
		List<String> mapKey = partsMap.entrySet().stream()
												 .map(m -> String.valueOf(m.getKey()))
												 .collect(Collectors.toList());
		List<String> mapValue = partsMap.entrySet().stream()
												   .map(m -> m.getValue())
												   .collect(Collectors.toList());
			//TreeMapに変換する
